...
// add = just update sum memory, spawn = account for game behavior
...
struct Crop
{
	...
	uvec3 coords;
	uint yield;
	float time_to_maturity; // should this be an int?
};

void add_crop(Crop* crops, uvec3 pos, uint type)
{
...
void update(Crop* crops, Chunk* chunks, float dtime)
{
	for (uint i = 0; i < MAX_CROPS; i++)
	{
		if (crops[i].type != NULL)
...
{
	load(&renderer->crop_mesh, "assets/meshes/crop.model", sizeof(renderer->crops));
